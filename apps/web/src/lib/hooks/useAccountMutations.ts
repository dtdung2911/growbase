...
import type {
  CreateAccountInput,
  UpdateAccountInput,
} from "@growbase/shared/schemas/account-settings"
...
export function useCreateAccount() {
  ...
  return useMutation({
    mutationFn: async (input: CreateAccountInput): Promise<Account> => {
      ...
      if (!res.ok) throw new Error(json.error ?? "Không thêm được tài khoản")
      return json.data
    },
    onSuccess: () => {
      if (householdId) {
      ...
      toast.success("Đã thêm tài khoản", { duration: 2000 })
    },
    onError: (err: Error) => {
