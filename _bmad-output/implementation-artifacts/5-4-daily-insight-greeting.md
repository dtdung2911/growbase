...
| 6 | ~~`activeGoalFund.monthly_contribution` reaches the insight sentence~~ | **[Code review 2026-07-06]** `monthlyContribution` param removed from `buildInsightDescriptor()` — no `insight.*` i18n template ever consumed it (dead value); dropped to fix a broken-sentence bug when household has no goal/emergency fund (`goalName` fallback `""`). `goalName` still reaches the sentence via `underPlanYesterday`/`underPlanYesterdayNoGoal`. | ⚠️ Stale — param removed |
