...
- [x] Task 1: Mở rộng `updateFundSchema` (AC: 2)
  ...
  - [x] Unit tests: accept/reject bounds `target_months_expense` + icon string free (fund.test.ts +4 tests mới → 22 total).
...
- `npx vitest run` → 413 tests pass / 31 files; `fund.test.ts` 22 pass (+4 tests mới, 18→22).
...
- MODIFIED: `src/__tests__/validations/fund.test.ts` — +4 tests mới (18→22) bounds + icon string.
...
- 09-07-2026 — Code-review fixes: fund.ts thêm `.int()` months + `.trim()` name + icon `.max(120)`/regex iconify; FundEditSheet render lỗi từng field (name/months/amount), months NaN→null (bỏ `||0`), date-future chỉ validate khi dirty, D1 recompute emergency target + preview, icon picker radiogroup/radio aria-checked; useUpdateFund toast → `t()`; `fund-detail` literal → `keys.fundDetail` factory; thêm i18n `funds.priority` + 6 key mới (vi+en, parity 773==773); fund.test.ts +5 tests (22→27). tsc exit 0, vitest 419 pass/31 files.

### Review Findings (code review 09-07-2026)

- [x] [Review][Decision] D1: Emergency months edit là no-op nhìn thấy được. QUYẾT ĐỊNH (user chốt): recompute `target_amount` client-side = `Math.round(monthsMới × (fund.target_amount / fund.target_months_expense))` gửi kèm PATCH; nếu `target_months_expense` cũ null/0 → fallback chỉ gửi months, không recompute. Thêm preview "Mục tiêu mới" (text-xs, font-mono) dưới input months để user thấy tác dụng.
- [x] [Review][Patch] target_months_expense thiếu .int() — 6.5 tháng pass PATCH (AC ghi "int dương") [src/lib/validations/fund.ts:29]
- [x] [Review][Patch] updateFundSchema.icon string tự do KHÔNG bound — authenticated user PATCH string megabyte vào funds.icon, iconify lookup arbitrary. Thêm .max + regex format iconify (giữ tương thích lucide cũ) [src/lib/validations/fund.ts]
- [x] [Review][Patch] updateFundSchema.name không .trim() — "   " persist thành tên rỗng nhìn thấy (onboarding cùng epic đã trim) [src/lib/validations/fund.ts]
- [x] [Review][Patch] FundEditSheet fail im lặng mọi field trừ date: name required / months min-max / amount min block submit không render error, không toast; clear months → valueAsNumber||0 = 0 → Save no-op không feedback. Vi phạm error pattern dự án [src/components/funds/FundEditSheet.tsx]
- [x] [Review][Patch] Date-future validation chặn CẢ rename/re-icon khi target_date quá khứ hoặc null (goal quá hạn = trạng thái bình thường) — chỉ validate khi user thay đổi date [src/components/funds/FundEditSheet.tsx:59-62]
- [x] [Review][Patch] useUpdateFund toast hardcode tiếng Việt ("Đã cập nhật" / "Không cập nhật được quỹ") — EN user thấy VN toast trên chính flow AC5 audit [src/lib/hooks/useFunds.ts]
- [x] [Review][Patch] "fund-detail" query key hardcode literal, không có trong keys factory — vi phạm rule #5 CLAUDE.md [src/lib/queries/queryKeys.ts + src/lib/hooks/useFunds.ts]
- [x] [Review][Patch] FundEditSheet icon picker mất radiogroup/aria-checked semantics mà GoalStep (9.2) đã có — thống nhất structure [src/components/funds/FundEditSheet.tsx]
- [x] [Review][Patch] Story doc: Debug Log ghi "+8 tests mới" — thực tế +4 (18→22) [9-3 story file]
- [x] [Review][Patch] Baseline: key `funds.priority` được page dùng nhưng thiếu ở CẢ vi.json + en.json → render raw key [src/app/(app)/funds/[id]/page.tsx:310]
