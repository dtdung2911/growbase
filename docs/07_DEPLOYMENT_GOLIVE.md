...
- dnsutils (cung cấp `dig`/`nslookup` để verify DNS §10.2, §7.5) — Ubuntu minimal KHÔNG có sẵn

Cài package nền:

```bash
sudo apt update
# git=clone repo (§4/§7.3) · nginx=reverse proxy (§7.5/§11) · curl=NodeSource+smoke test
# build-essential=biên dịch native module khi npm ci (§8) · dnsutils=dig/nslookup verify DNS (§10.2/§7.5)
sudo apt install -y git nginx curl build-essential dnsutils
```
...
Nếu repo **private**, `git clone` sẽ hỏi credential (hoặc fail nếu chạy non-interactive). Xác thực bằng 1 trong 2 cách trước khi clone:

- **HTTPS + Personal Access Token (PAT):** `git clone https://<user>:<PAT>@github.com/<org>/<repo>.git growbase` (tạo PAT read-only ở GitHub → Settings → Developer settings). Muốn khỏi nhập lại: `git config --global credential.helper store` sau lần đầu.
- **SSH deploy key:** tạo key trên server `ssh-keygen -t ed25519 -C growbase-deploy -f ~/.ssh/growbase_deploy` (Enter bỏ qua passphrase), thêm nội dung `~/.ssh/growbase_deploy.pub` vào repo → Settings → Deploy keys (read-only), rồi `git clone git@github.com:<org>/<repo>.git growbase`. Verify: `ssh -T git@github.com`. (`ssh`/`ssh-keygen` có sẵn trên Ubuntu qua openssh-client.)
...
Khuyến nghị apply bằng Supabase CLI đã link tới production project. **Supabase CLI KHÔNG có sẵn trên server trắng** — chọn 1: (a) cài trên server (tải bản `.deb` mới nhất ở `https://github.com/supabase/cli/releases` rồi `sudo dpkg -i supabase_*.deb`; hoặc Homebrew), (b) chạy CLI ngay trên máy dev local (nơi đã có repo + CLI), hoặc (c) bỏ CLI, apply thủ công trong SQL Editor như ghi chú cuối mục.
...
```bash
# Self-host §7 chạy TRƯỚC §12 nên certbot có thể chưa cài — cài ở đây (apt idempotent, chạy lại vô hại)
...
**Cách A — Registrar bất kỳ (Namecheap/GoDaddy/Cloudflare...):** vào phần DNS / Advanced DNS của domain → thêm 2 bản ghi A như trên. Với Cloudflare: tắt proxy (mây xám) khi chạy certbot lần đầu, bật lại sau nếu muốn (nếu certbot trả 403 vì proxy còn bật, hoặc lỗi NXDOMAIN: xem §18).
...
(Cần Supabase CLI — xem ghi chú Supabase CLI ở §6.2. Không có CLI thì áp qua SQL Editor (§6.2), hoặc `psql`/`docker exec` (§7.6) nếu self-host.)
...
### Certbot fail: Cloudflare proxy chặn HTTP-01 (403 unauthorized)

Triệu chứng khi chạy `sudo certbot --nginx -d <domain>`:

```text
Type: unauthorized
Detail: ... Invalid response from http://<domain>/.well-known/acme-challenge/... : 403
```

Nguyên nhân: domain đặt trên Cloudflare và bản ghi đang bật **proxy (mây cam)**. HTTP-01 challenge của Let's Encrypt không trúng Elastic IP của EC2 mà trúng edge Cloudflare (IP dạng `104.x.x.x` / `172.67.x.x`, hoặc IPv6 `2606:4700:...`) → Cloudflare trả 403 cho đường dẫn `acme-challenge`.

Sửa:

1. Cloudflare dashboard → DNS → mở bản ghi A của domain (và `www`, và `api` nếu self-host §7.5) → chuyển sang **DNS only (mây xám)**.
2. Nếu EC2 KHÔNG có IPv6: **xoá bản ghi AAAA** (nếu có) — certbot ưu tiên IPv6, trỏ về địa chỉ không phục vụ được cũng làm fail.
3. Verify đã trỏ thẳng về server (không phải Cloudflare):

```bash
dig +short <domain>      # phải ra Elastic IP (§3.3), KHÔNG phải 104.x/172.67.x/IPv6 Cloudflare
```

4. Chạy lại certbot khi `dig` ra đúng Elastic IP:

```bash
sudo certbot --nginx -d <domain>
```

Sau khi cấp cert xong:

- **Subdomain API self-host (§7.5): để nguyên DNS-only.** Cloudflare proxy chặn WebSocket của Supabase Realtime → app mất realtime nếu bật mây cam.
- Nếu vẫn muốn bật lại proxy (mây cam) cho domain app: BẮT BUỘC đặt SSL/TLS mode = **Full (strict)** trong Cloudflare, nếu không sẽ lặp redirect / lỗi cert. Cross-ref §10.1.

### Certbot fail: DNS NXDOMAIN (bản ghi A chưa có / chưa propagate)
...
Nguyên nhân: bản ghi A của domain/subdomain chưa được tạo, hoặc mới tạo và chưa propagate → Let's Encrypt không resolve được domain về IP nào.

Sửa:

1. Tạo bản ghi A: Host = domain/subdomain → Value = Elastic IP (§3.3). Xem §10.1 (domain app) và §7.5 (subdomain API self-host).
2. Verify đã resolve; chờ propagate nếu chưa ra:

```bash
dig +short <domain>      # phải ra Elastic IP; rỗng = chưa propagate, chờ thêm (TTL 300 ~ vài phút)
```

3. CHỈ chạy certbot khi `dig` trả đúng Elastic IP **và** Security Group đã mở cổng 80 (§3.2). Đây là cùng điều kiện tiên quyết ở §10 và §12 — certbot HTTP-01 fail nếu thiếu 1 trong 2.
