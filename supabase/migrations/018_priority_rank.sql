-- Description: Story 12.1 — hạng quỹ sống (priority_rank) sống trong DB.
-- 1. funds.priority_rank int nullable (chỉ goal funds có rank; emergency/khác = NULL).
-- 2. Backfill goal funds mỗi household = 1,2,3… theo created_at.
-- 3. RPC re-base trên 017 (KHÔNG phải 016) — giữ NGUYÊN 100% thân 017 (đặc biệt cột `icon` từ
--    story 9.2). Thay đổi DUY NHẤT so với 017: funds INSERT thêm cột priority_rank theo thứ tự
--    p_goals (emergency [0] = NULL, goal thứ n = n).
--
-- QUYẾT ĐỊNH D1 (flag reviewer): KHÔNG reuse cột `priority` (DEFAULT 5) / `sort_order` (DEFAULT 0).
-- Hai cột đó là nghĩa "UI sort chung mọi fund" (/api/funds order by priority, sort_order) — default
-- không phản ánh ladder hạng. Cột mới `priority_rank` đúng ngữ nghĩa BR-OB-016 (USER xếp hạng quỹ
-- sống), tránh đổi nghĩa ngầm cột cũ.

BEGIN;

ALTER TABLE funds ADD COLUMN IF NOT EXISTS priority_rank int;

-- Backfill: chỉ đụng goal funds chưa có rank (idempotent — chạy lại là no-op).
-- Tie-breaker `, id`: onboarding INSERT các fund trong CÙNG transaction → created_at IDENTICAL,
-- ORDER BY created_at đơn lẻ không deterministic. `, id` cho thứ tự ổn định. LIMITATION: khi tie,
-- đây KHÔNG khôi phục đúng thứ tự user chọn lúc onboarding (không lưu lại được) — user re-rank ở 12.2.
WITH ranked AS (
  SELECT id,
         ROW_NUMBER() OVER (PARTITION BY household_id ORDER BY created_at, id) AS rn
  FROM funds
  WHERE fund_type = 'goal' AND priority_rank IS NULL
)
UPDATE funds f
...
BEGIN
  ...
  LOOP
    INSERT INTO funds (
      household_id, name, fund_type, target_amount, target_date, target_months_expense, icon, priority_rank
    ) VALUES (
      v_household_id,
      v_item->>'name',
      ...
      NULLIF(v_item->>'icon', ''),
      CASE WHEN v_item->>'fund_type' = 'emergency' THEN NULL ELSE v_idx END
    )
...
-- Lockdown (giữ pattern 012/013/016/017): chỉ service_role (API route đã auth) được gọi.
