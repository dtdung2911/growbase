-- Description: Harden complete_onboarding_v2 (code review 09-07-2026).
-- Thay đổi so với 013:
--   - Advisory lock đầu body → serialize double-submit (EXISTS check khi đó mới đáng tin).
--   - Guard chặt: đúng 1 emergency ở [0], cap ≤ 6 quỹ, mọi target_amount > 0.
--   - Localize tên do server tạo (household/income/account) qua param theo locale user.
--   - Trả jsonb { household_id, fund_ids } để API map id từng quỹ.
-- Signature đổi (thêm param, đổi return type) → DROP trước, không CREATE OR REPLACE được.
