-- Description: Story 8.1 (multi-goal) — complete_onboarding_v2 giờ nhận p_goals (mảng).
-- Quỹ khẩn cấp LUÔN là nền tảng (p_goals[0], route đảm bảo), kèm N quỹ mục tiêu, tất cả
-- trong CÙNG transaction (rule 1: fund ops atomic all-or-nothing).
-- Signature type không đổi (uuid, text, numeric, jsonb, jsonb) — chỉ đổi TÊN param cuối
-- p_goal → p_goals. CREATE OR REPLACE không đổi được tên param → phải DROP trước.
-- SECURITY DEFINER SET search_path = public, pg_temp (rule tuyệt đối từ S0).

BEGIN;

DROP FUNCTION IF EXISTS complete_onboarding_v2(uuid, text, numeric, jsonb, jsonb);
...
BEGIN
  ...
  END IF;
  IF p_goals->0->>'fund_type' <> 'emergency' THEN
...
-- Lockdown (giữ nguyên pattern 012): chỉ service_role (API route đã withAuth) được gọi.
